...
# jpgs from https://www.albumrock.net/groupe-pink-floyd-252.html
# extracted with https://extract.pics
# magick mogrify -posterize 10 -path . -format PNG *.jpg

library(colorfindr)
fls = list.files("sandbox/pinkfloyd", pattern = "*.PNG", full.names = T)

res = lapply(fls, function(f) {
	x = colorfindr::get_colors(f)
	pal = colorfindr::make_palette(x, n = 8, show = F)
})
dput(res)

# only 'moon' manually adjusted (rainbow)
