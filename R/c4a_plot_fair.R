...
c4a_plot_fair = function(pal, dark = FALSE, contours = seq(10, 90, by = 10)) {
	grid::grid.newpage()
	...
	m = matrix(df$LC, nrow = length(dL))[length(dL):1L, ]
	...
	cellplot(2,2, {
		...
		#grid::grid.raster(m, x = 0.5, y = 0.5, width = 1, height = 1)
