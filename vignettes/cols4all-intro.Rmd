...
* **HCL Analysis** Does a palette work well for statistical purposes? We analyse this with the HCL (hue-chroma-luminance) color space.
...
* **Contrast** What is the contrast between the colors in the palette? We focus on two questions: how well is text readable (also for people with a visual impairment)? And: are border lines needed to separate colors?
...
The `cols4all` package is available on github, and soon also on CRAN:
